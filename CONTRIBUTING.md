...
Those four commands are also this repository's own deterministic gates, declared in `.orchestrator/verification.json`. A project defines its checks there; the engine reads that file as committed at a task's base revision, so an implementing agent cannot weaken the gates that judge its own work. A repository without the file keeps the previous detected Cargo and Omarchy behavior, and a file the engine cannot use is a visible infrastructure error rather than a silent fall back. See [ADR-0017](docs/adr/0017-read-project-verification-policy-from-the-task-base-revision.md).
...
The current run survives engine or shell restarts. The task-centered panel keeps bounded durable activity and pause, resume, cancel, redirect, or context controls with the selected task, preserving partial worktree changes and linked attempt history. A paused process remains owned by the live engine and is recovered as interrupted if the engine exits. `finish` records the project's deterministic checks, sends their evidence to an independent reviewer, runs a bounded fresh correction session when needed, and prepares the exact final tree for contextual task inspection. That inspection shows the complete patch, changed paths, gate status, and proposed one-task commit. A separate `approve` revalidates that tree and creates the local task commit; `reject` preserves the result without committing. `integrate` is another explicit action that only fast-forwards an existing checked-out clean local branch to the approved commit. Semantic multi-commit splitting, divergent merges, and push actions remain to be implemented. See [ADR-0015](docs/adr/0015-center-the-panel-on-tasks.md).
