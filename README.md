...
>
> **Project status:** The foundation and planning slice are implemented. The repository contains a Rust engine, shared versioned IPC types, SQLite-backed run state, bounded local Git repository discovery, authenticated GitHub CLI discovery and explicit cloning, constrained Codex and Claude Code planner adapters, a CLI, and reconnecting Omarchy bar-widget and panel entry points. The panel supports a keyboard-driven Local/GitHub repository browser with shared search and manual-path fallback, durable draft creation, structured plan generation, task editing and reordering, and plan approval or rejection. From the CLI, an approved task can receive an isolated worktree and a user-selected Codex or Claude implementer supervised by the engine. The Omarchy Overview shows bounded durable implementation activity and can pause, resume, cancel, redirect, or add context while preserving partial work and linked attempt history. The CLI can run a durable structured review in a fresh session, preferring the provider that did not implement the task and recording any permitted same-provider fallback explicitly. Deterministic verification, review correction loops, Review-panel presentation, and final change approval remain planned.
>
...
The implemented planning slice keeps that choice explicit. The engine launches the selected authenticated CLI non-interactively with read-only planning permissions, sends its prompt over standard input, requires a shared structured plan schema, and validates the result before storing it. It does not select a model or use a direct model API. Planner processes and captured output are bounded, and failures remain inspectable and retryable. The initial implementation adapters use the same authenticated CLIs, but grant writes only inside an existing task worktree, remain engine-supervised, and do not cross verification, review, or commit boundaries. Their categorized activity is persisted and exposed as a bounded recent snapshot window. Pause and resume signal the supervised process group without consuming timeout budget; cancellation preserves partial work; redirects and added context stop the current non-interactive process and launch a durable linked continuation in the same worktree. A completed implementation can be reviewed from the CLI by a fresh, read-only process from the other provider. An explicit policy may fall back to the implementer's provider only when the preferred CLI cannot launch; that weaker independence is recorded durably. Review verdicts and findings are structured and persisted, but approval remains technical task evidence rather than permission to commit or push. See [ADR-0004](docs/adr/0004-run-planners-as-constrained-cli-processes.md), [ADR-0007](docs/adr/0007-supervise-implementation-agents-in-task-worktrees.md), [ADR-0008](docs/adr/0008-persist-bounded-implementation-activity-and-cancel-process-groups.md), [ADR-0009](docs/adr/0009-control-and-continue-running-implementations.md), and [ADR-0010](docs/adr/0010-run-independent-reviews-in-fresh-agent-sessions.md).
