...
The persistent task rail keeps the selected project, original task, and current workflow stage visible. Before work starts, those stages distinguish a task still at **Plan**, a generated proposal that is **Planned**, and an approved task that is **Ready to run**. The same rail exposes project choice and switching without requiring a hidden setup view.
