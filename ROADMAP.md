...
- [x] Define project verification commands and policy without asking an agent to infer success. The engine reads `.orchestrator/verification.json` as committed at the task's base revision, so an implementing agent cannot weaken its own gates. See [ADR-0017](docs/adr/0017-read-project-verification-policy-from-the-task-base-revision.md).
