...
pub(crate) fn command(request: &AgentRequest<'_>) -> Result<std::process::Command, String> {
    ...
    if !matches!(
        role,
        "architect" | "architect_review" | "chat" | "enhance" | "model_policy" | "planner" | "response_correction" | "implementer" | "fixer" | "reviewer"
    ) {
        return Err("unknown agent role".into());
    }
    let review = matches!(role, "architect_review" | "reviewer");
    let readonly = matches!(role, "architect" | "chat" | "enhance" | "model_policy" | "planner" | "response_correction") || review;
...
pub(crate) fn verify_capabilities(
    ...
    if !matches!(request.role, "architect" | "architect_review" | "reviewer" | "chat" | "enhance" | "model_policy" | "planner" | "response_correction") {
        return Ok(());
    }
