...
impl Ctx {
    ...
    pub(crate) fn chat_worker(&self, current_plan: &Value, question: &str) {
        ...
        let result = (|| -> Result<(), String> {
            ...
            let readonly_prompt = format!("{prompt}\nOUTPUT CONTRACT OVERRIDE: read-only Q&A in a fresh conversation. Do not write files or alter plan/decisions. Return ONLY {{\"answer\":\"your answer\"}}. Saved architecture context: {}\nProject synthesis: {}", crate::prompt_view::checkpoint_for(&context, current_plan), self.synthesis_context(Some(current_plan)));
